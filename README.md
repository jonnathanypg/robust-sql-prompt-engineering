...
---
soning` | `string` | Lógica aplicada o razón del rechazo |

### 4.2 Reglas de Seguridad (9 Reglas)

1. **READ-ONLY ACCESS** - Solo SELECT permitido
2. **SQL INJECTION PREVENTION** - Detecta patrones maliciosos
3. **DOMAIN BOUNDARIES** - Solo consultas de inventario
4. **TABLE RESTRICTION** - Solo tabla `inventory`
5. **SCOPE RESTRICTION** - Protege métricas de negocio
6. **QUERY SAFETY** - LIMIT obligatorio
7. **DIALECT COMPATIBILITY** - SQL estándar portable
8. **CASE INSENSITIVITY** - Búsquedas con `LOWER()`
9. **NO HALLUCINATION** - No inventa columnas

### 4.3 Mapeo de Categorías de Vehículos

| Término del Usuario | Mapeo SQL | Incluye |
|---------------------|-----------|---------|
| "Auto" / "Coche" | `sedan` OR `hatchback` | Vehículos de pasajeros |
| "Camioneta" / "Pickup" | `truck` | Solo pickups |
| "SUV" / "Todoterreno" | `suv` | Solo SUVs |
| "Carro" / "Vehículo" | Sin filtro | Todo el inventario |

---

## 5. Guía de Ejecución (Entorno de Pruebas)
...
Importa el archivo `seed_data.sql` para crear la tabla `inventory` con datos de prueba:
...
## 6. Pruebas de Seguridad (SQL Injection)

Puedes probar la robustez del sistema contra ataques de inyección SQL:
...
Todos los ataques deben ser rechazados con:
...
### 7.1 Prevención de Alucinaciones

El prompt define explícitamente las **columnas inexistentes** para evitar que el modelo invente datos:

- ❌ No existe: `color`, `kilometraje`, `combustible`, `transmisión`
- ❌ No existe: tabla `customers`, `sales`, `transactions`
- ❌ No existe: fecha de venta (solo año de fabricación)

### 7.2 Mapeo Estricto de Categorías

Se define que "Auto" ≠ "Vehículo genérico":
- **"Auto"** = Solo Sedan y Hatchback
- **"Carro"** = Término genérico (muestra todo)

### 7.3 JSON Puro (Headless Mode)

El prompt está blindado para que el LLM actúe como una **API silenciosa**, retornando únicamente JSON válido sin:
...
├── demo.py             # Demo interactivo End-to-End
├── agent.py            # Agente SQL (genera JSON)
├── consumer.py         # Consumidor (ejecuta SQL)
├── seed_data.sql       # Datos de prueba para la tabla inventory
├── requirements.txt    # Dependencias Python
